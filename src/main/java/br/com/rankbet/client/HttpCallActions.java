...
public class HttpCallActions {
    ...
    public static void GET( String endpoint, CloseableHttpClient httpClient ) {
        ...
        try {
            URI uri = new URIBuilder( endpoint ).setParameter( "page", "2" ).build();
    ...
    private static SSLConnectionSocketFactory getSSLContext() {
        ...
        try {
            connFactory = new SSLConnectionSocketFactory( SSLContextBuilder.create().loadTrustMaterial( trustStrategy ).build(), allVerifier );
